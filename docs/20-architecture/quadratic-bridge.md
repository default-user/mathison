...
- **Web developers** building browser-based OI applications needing system-level access
- **Security engineers** auditing the bridge's authentication and authorization model
- **DevOps teams** deploying the bridge in production with proper isolation
- **Mobile developers** using the bridge for capability escalation from browser stage
- **Integration developers** connecting browser OIs to mesh networks

## Why This Exists

Browser-based OIs run at BROWSER stage with limited capabilities (no system.exec, network access, or file I/O). The Quadratic Bridge solves this by running a full OI runtime at SYSTEM/NETWORK stage, exposing governed HTTP endpoints that browser OIs can call to access privileged operations. This enables web-based OI applications without compromising Quadratic's governance model.

## Guarantees / Invariants

**Security:**
- All endpoints (except /status) require API key authentication by default
- Only actions in the allowlist can be relayed (deny-by-default)
- Rate limiting: 100 req/min per client (configurable)
- Constant-time authentication using SHA-256 to prevent timing attacks
- System actions (system.exec/read/write) disabled by default, require explicit BRIDGE_ALLOW_SYSTEM=true

**Governance:**
- Bridge OI inherits Quadratic's CDI gating (only NETWORK stage actions allowed)
- All actions logged with receipts in hash chain
- CIF boundary enforcement: input sanitization, output redaction
- Anti-hive enforcement: each browser OI tracked separately, no state fusion
- Posture: Bridge runs at HIGH posture for stricter enforcement

**Operational:**
- CORS origin allowlist prevents unauthorized cross-origin requests
- Input sanitization: max 3 levels nesting, 10KB strings, 100 array items
- Audit logging: last 1000 events retained in memory
- All mesh communication uses BeamEnvelope (encrypted, taint-labeled)

## Non-Goals

- Generic HTTP proxy (OI-specific bridge only)
- Persistent browser session state (stateless relay)
- Client-side governance enforcement (server-side only)
- Public internet exposure without reverse proxy (localhost binding recommended)
...
curl -X POST http://localhost:3142/dispatch \
  -H "Content-Type: application/json" \
  -d '{"action":"llm.complete","args":{}}'
# Should return 401 Unauthorized

# Verify auth succeeds with valid key
export API_KEY=$(openssl rand -hex 32)
BRIDGE_API_KEY=$API_KEY npx tsx quadratic-bridge.mjs &
curl -X POST http://localhost:3142/dispatch \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action":"llm.complete","args":{"prompt":"test"}}'
...
**Action Allowlist:**
```bash
# Verify system actions are blocked by default
curl -X POST http://localhost:3142/dispatch \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action":"system.exec","args":{"command":"whoami"}}'
...
curl -X POST http://localhost:3142/dispatch \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action":"llm.complete","args":{"prompt":"hello"}}'
# Should return 200 OK
```
...
curl http://localhost:3142/verify
# Should return {"valid":true,"receipts_verified":N,"errors":[]}
```

**CORS Policy:**
```bash
# Verify CORS enforcement
curl -H "Origin: https://evil.com" http://localhost:3142/dispatch
# Should return CORS error

# Verify allowed origins work
BRIDGE_ALLOWED_ORIGINS="https://app.example.com" npx tsx quadratic-bridge.mjs &
curl -H "Origin: https://app.example.com" http://localhost:3142/status
...
**Bridge Server:**
- Main file: `quadratic-bridge.mjs`
- Server framework: Node.js http module (no dependencies)
- OI runtime: Quadratic Monolith v0.2.0 (embedded in bridge process)

**Action Allowlist:**
- Location: `quadratic-bridge.mjs`, `ACTION_ALLOWLIST` constant
- Structure: `{action: "name", risk: "LOW|MEDIUM|HIGH|CRITICAL"}`
- To add action: Add entry to allowlist, consider risk level
- System actions: Only enabled if `BRIDGE_ALLOW_SYSTEM=true`

**Rate Limiting:**
- Implementation: In-memory map of client ID → request timestamps
- Window: Sliding 60-second window
- Key: `X-OI-ID` header or IP address fallback
- Reset: Automatic garbage collection of expired timestamps

**Authentication:**
- Method: SHA-256 hash comparison (constant-time)
- Header: `X-API-Key`
- Storage: Environment variable `BRIDGE_API_KEY`
- Bypass: Set `BRIDGE_REQUIRE_AUTH=false` (dev only)

**Audit Logging:**
- Storage: In-memory circular buffer (last 1000 events)
- Format: `{timestamp, client_id, action, args, result, error}`
- Endpoint: `GET /audit?count=N` (auth required)
- Persistence: None (restart clears log)

**Mesh Integration:**
- BeamEnvelope: Standard Quadratic mesh protocol
- Peer registry: In-memory map of OI ID → {address, stage}
- Message routing: Bridge acts as relay between browser OIs
- Encryption: Not yet implemented (planned for v0.4.0)

**Adding new endpoints:**
1. Add route handler in `quadratic-bridge.mjs`
2. Apply auth middleware if needed
3. Sanitize inputs using `sanitizeInput()`
4. Dispatch to bridge OI via `oi.dispatch(action, args)`
5. Return result with proper status code

**Testing locally:**
- Start bridge: `npx tsx quadratic-bridge.mjs`
- Open browser: `file:///path/to/quadratic.html`
- Connect to bridge: Enter `http://localhost:3142`
- Test actions: Use UI buttons or curl commands
