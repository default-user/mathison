...
- **Contributors** navigating the codebase and understanding component relationships
- **System architects** evaluating the distributed mesh design
- **Mobile developers** implementing mobile-specific OI features
- **SDK maintainers** working with multi-language client generation
- **Security reviewers** auditing the governance integration points

## Why This Exists

Mathison's architecture spans single-node personal OI through distributed mesh computation, with governance enforced at every layer. This document maps the component responsibilities, data flows, and integration points across the full stack—from CIF/CDI kernel through OI Engine, Memory Graph, ModelBus, and MeshCoordinator. Understanding these boundaries is essential for extending the system without breaking governance guarantees.

## Guarantees / Invariants

**Governance Integration:**
- Every component respects treaty rules (CIF boundary enforcement, CDI fail-closed kernel)
- No component can bypass governance to access tools, memory, or I/O
- ModelBus enforces anti-hive rules: no identity fusion across nodes
- MeshCoordinator requires explicit consent for mesh formation

**Data Isolation:**
- Personal nodes maintain local memory (no raw memory sharing between mesh nodes)
- Inference requests encrypted in transit across mesh
- Mesh formation requires encryption enforcement and node allowlisting

**Architectural:**
- Single node deployment: one Mathison instance per device (phone/laptop/server)
- Distributed mesh: temporary computational mesh with automatic dissolution
- No persistent mesh state across sessions

## Non-Goals

- Multi-node persistent state synchronization (offline-first, local-only memory)
- Generic distributed compute framework (OI-specific mesh protocol)
- Centralized mesh coordinator (peer-to-peer formation)
- Cross-instance memory sharing without explicit mesh participation
...
```bash
# Start Mathison server
cd packages/mathison-server
pnpm start

# Verify all layers respond
curl http://localhost:3000/status
# Should show: CIF, CDI, OI Engine, Memory Graph, ModelBus, MeshCoordinator

# Test local interpretation
curl -X POST http://localhost:3000/oi/interpret \
  -H "Content-Type: application/json" \
  -d '{"message":"Hello Mathison"}'
```

**Mesh Formation:**
```bash
# Start three nodes on different ports
MATHISON_PORT=3001 pnpm start &
MATHISON_PORT=3002 pnpm start &
MATHISON_PORT=3003 pnpm start &

# Form mesh from node 1
curl -X POST http://localhost:3001/mesh/form \
  -H "Content-Type: application/json" \
  -d '{"peers":["http://localhost:3002","http://localhost:3003"]}'

# Verify mesh status
curl http://localhost:3001/mesh/status
# Should show 3 connected nodes
```

**Ensemble Synthesis:**
```bash
# Request distributed inference across mesh
curl -X POST http://localhost:3001/modelbus/synthesize \
  -H "Content-Type: application/json" \
  -d '{
...
# Response should include contributors and quality scores
```

**Governance Enforcement:**
```bash
# Verify CDI blocks unapproved actions
curl -X POST http://localhost:3000/system/exec \
  -H "Content-Type: application/json" \
  -d '{"command":"rm -rf /"}'
# Should return 403 Forbidden with treaty violation

# Verify CIF sanitizes inputs
curl -X POST http://localhost:3000/oi/interpret \
  -H "Content-Type: application/json" \
  -d '{"message":"<script>alert(1)</script>"}'
# Should sanitize script tags before processing
```

## Implementation Pointers

**Component Locations:**
- CIF: `packages/mathison-governance/src/cif.ts`
- CDI: `packages/mathison-governance/src/cdi.ts`
- OI Engine: `packages/mathison-oi/src/oi-engine.ts`
- Memory Graph: `packages/mathison-memory/src/memory-graph.ts`
- ModelBus: `packages/mathison-mesh/src/model-bus.ts`
- MeshCoordinator: `packages/mathison-mesh/src/mesh-coordinator.ts`

**Adding a new ModelBus routing strategy:**
- File: `packages/mathison-mesh/src/model-bus.ts`
- Interface: `RoutingStrategy` with `selectModel(request, availableModels)` method
- Register strategy in `ModelBus.constructor()` strategies map
- Example strategies: `PreferenceRouting`, `LoadBalancedRouting`, `LatencyAwareRouting`

**Extending MeshCoordinator for new discovery methods:**
- File: `packages/mathison-mesh/src/mesh-coordinator.ts`
- Add discovery method to `DiscoveryMethod` enum
- Implement discovery logic in `MeshCoordinator.discover()` switch
- Current methods: `PROXIMITY`, `BROADCAST`, `MANUAL`

**Implementing a new OI interpretation strategy:**
- File: `packages/mathison-oi/src/oi-engine.ts`
- Interface: `InterpretationStrategy` with `interpret(input, context)` method
- Return: `{output, confidence, alternatives[]}`
- Register in `OIEngine.strategies` map

**ModelBus ensemble synthesis:**
- Entry point: `ModelBus.synthesize(request, ensembleSize)`
- Selection: Uses diversity-based model selection across nodes
- Synthesis: `CompositionalSynthesizer.merge(results[])`
- Quality: Weighted average based on per-model quality scores
- See: `packages/mathison-mesh/src/model-bus.ts`, `compositional-synthesizer.ts`

**Testing mesh privacy guarantees:**
- Test suite: `packages/mathison-mesh/tests/privacy.test.ts`
- Verifies: No raw memory sharing, encrypted requests, consent enforcement
- Run: `cd packages/mathison-mesh && pnpm test privacy.test.ts`
