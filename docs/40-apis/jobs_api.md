...
- **Application developers** orchestrating long-running tasks (data transforms, analysis, batch processing)
- **DevOps engineers** building resilient workflows that survive crashes/restarts
- **API integrators** needing deterministic, auditable job execution
- **Reliability engineers** implementing fault-tolerant systems with automatic resume

If you're just storing/querying data, use the Memory API instead.

---

## Why This Exists

**Problem:** Stateless APIs fail during long-running operations; retries cause duplicate work; no auditability.

**Solution:** The Jobs API provides:
- **Checkpointing**: Jobs save progress at stage boundaries; resume from last checkpoint after crash
- **Idempotency**: Explicit job IDs prevent duplicate execution on retry
- **Receipts**: Cryptographic audit trail with genome metadata for all state changes
- **Governance**: All actions protected by CIF+CDI (same as Memory API)

**Design choice:** Jobs are first-class entities with persistent state, not ephemeral request handlers.

---

## Guarantees / Invariants

1. **Checkpoint determinism**: Same inputs + same completed stages → same outputs (no side effects replayed)
2. **Idempotency**: Retrying with same `job_id` is safe; no duplicate work or receipts
3. **Fail-closed**: Jobs denied if governance not initialized or CDI denies action
4. **Receipt completeness**: Every state transition produces a receipt with genome metadata
5. **Bounded execution**: All jobs subject to timeout (default 30s); no runaway processes
6. **Stage atomicity**: Stages complete fully or not at all (no partial checkpoints)

---

## Non-Goals

- **Distributed job execution**: Single-server only (no worker pools or job queues)
- **Job scheduling**: No cron-like scheduling; jobs are triggered by API calls
- **Inter-job dependencies**: No DAG/workflow orchestration (use external tools like Temporal/Airflow)
- **Streaming job outputs**: Use `/jobs/logs` for receipts; no streaming stdout/stderr
...
curl -X POST http://localhost:3000/jobs/run \
  ...
  -d '{"jobType": "test_job", "inputs": {}}'
# Expected: 200 with job_id, status "completed"
```
...
# Start a long job
curl -X POST http://localhost:3000/jobs/run \
  -d '{"jobType": "long_job", "jobId": "test-123"}' -H "Content-Type: application/json"
...
curl -X POST http://localhost:3000/jobs/resume \
  -d '{"job_id": "test-123"}' -H "Content-Type: application/json"
# Expected: Job continues from last checkpoint
```

### Verify idempotency
```bash
# Run twice with same job_id
curl -X POST http://localhost:3000/jobs/run -d '{"jobType": "test", "jobId": "idempotent-test"}' -H "Content-Type: application/json"
curl -X POST http://localhost:3000/jobs/run -d '{"jobType": "test", "jobId": "idempotent-test"}' -H "Content-Type: application/json"
# Expected: Second call returns existing job (no duplicate execution)
```

### Verify receipts
```bash
curl http://localhost:3000/jobs/logs?job_id=test-123
# Expected: Array of receipts with genome_id, genome_version, action, timestamp
```

### Verify timeout enforcement
```bash
curl -X POST http://localhost:3000/jobs/run \
  -d '{"jobType": "infinite_loop"}' -H "Content-Type: application/json"
# Expected: 408 after 30 seconds (or configured timeout)
```
...
**Location**: `/packages/mathison-server/src/routes/jobs.ts`

**Key components**:
- **JobExecutor**: Manages job lifecycle, checkpointing, and resumption
- **JobStore**: Persists job state to file system (default) or in-memory
- **CheckpointManager**: Saves/loads job state at stage boundaries
- **ActionGate integration**: All write operations produce receipts

**Job lifecycle**:
1. `POST /jobs/run` → JobExecutor creates job → checkpoints "init" stage
2. Job executes stages (init → process → finalize)
3. Each stage completion → checkpoint saved + receipt produced
4. Final stage → job marked "completed"
5. Crash during stage → resume from last checkpoint

**Storage**:
- Jobs stored at `/tmp/mathison/jobs/<job_id>.json`
- Receipts stored via ActionGate (same as Memory API)
