...
- Product managers planning mobile deployment strategy
- Mobile developers implementing React Native apps
- Business stakeholders evaluating mobile monetization
- Technical architects designing on-device ML systems

## Why This Exists

Provides a comprehensive deployment plan for bringing Mathison to mobile as a standalone Android app with on-device LLM capabilities. This guide bridges the gap between Mathison's existing TypeScript packages and mobile platform constraints, defining the technical stack, monetization model, and go-to-market strategy for a $365/year subscription service.

## Guarantees / Invariants

- **On-device inference**: All ML processing runs locally (no cloud dependency)
- **Privacy-first**: User data never leaves device without explicit consent
- **Reuse existing packages**: mathison-oi, mathison-memory, mathison-mesh work on mobile
- **Offline-first**: Core features work without internet connectivity
- **Governance intact**: All operations protected by Tiriti o te Kai framework

## Non-Goals

- iOS deployment (Android-first strategy)
- Cloud-based LLM inference (defeats privacy promise)
- Free tier with ads (incompatible with privacy model)
- Web-based mobile experience (native app required for on-device ML)
- Cross-platform framework other than React Native

## How to Verify

```bash
# 1. Verify React Native app builds
cd MathisonMobile
npx react-native run-android

# 2. Verify on-device model loads (requires Android 14+ device)
adb logcat | grep "Gemini Nano"
# Expected: "✓ Gemini Nano initialized"
...
# Check memory graph
# Expected: Previous messages retained

# 5. Verify subscription flow (after Play Store setup)
# Tap subscribe button
# Complete purchase flow
# Expected: Features unlocked, receipt stored
```

## Implementation Pointers

- **Phase prioritization**: Complete phases 1-3 (foundation, LLM, OI features) before mesh capabilities
- **Model selection**: Try Gemini Nano first (free, built-in), fallback to llama.cpp only if necessary
- **Storage layer**: Use `react-native-sqlite-storage` for graph persistence (same schema as Node.js version)
- **Native modules**: Implement `MobileMLKitModule.java` for Gemini Nano bridge
- **Subscription logic**: Use `com.android.billingclient` for Google Play Billing integration
- **Beta testing**: Start with 100 users at $99/year to validate product-market fit
- **Revenue target**: 1,000 users = $365k ARR minimum viable business
