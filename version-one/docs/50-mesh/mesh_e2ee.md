...
- **Mesh engineers** implementing secure peer-to-peer messaging
- **Cryptography reviewers** auditing handshake and encryption protocols
- **Security teams** verifying replay protection and key management
- **Integration engineers** connecting E2EE to discovery and storage layers

---

## Why This Exists

**Problem:** Mesh connections expose plaintext messages to network observers and enable replay attacks.

**Solution:** End-to-end encryption with mutual authentication ensures only authenticated peers can read messages, and replay protection prevents message reuse.

**Key principle:** Zero-knowledge network layer. Messages are opaque ciphertext; only session participants hold decryption keys.

---

## Guarantees / Invariants

1. **Mutual authentication**: Both peers verify identity before session establishment
2. **Forward secrecy**: Ephemeral session keys deleted after use; compromised long-term keys don't expose past sessions
3. **Replay protection**: Counter-based mechanism rejects duplicate or out-of-order messages
4. **No key leakage**: Private keys NEVER logged, serialized, or exposed in receipts
5. **AEAD encryption**: Authenticated encryption (AES-256-GCM or ChaCha20-Poly1305) prevents tampering
6. **Session isolation**: Each peer pair has independent session keys

---

## Non-Goals

- **Multi-party encryption** — sessions are pairwise; group chat requires separate layer
- **Key escrow** — no key recovery mechanism; lost keys = lost sessions
- **Online key verification** — no PKI or key servers; identity keys verified via genome attestation
- **Quantum resistance** — X25519 is not post-quantum; future upgrade path needed
- **Message ordering guarantees** — E2EE layer provides authenticity; ordering is application layer concern
...
```typescript
const message1 = alice.encrypt(Buffer.from('test'), aliceSession);
const message2 = alice.encrypt(Buffer.from('test'), aliceSession);

// Verify: message1 and message2 have different counters
assert.notEqual(message1.counter, message2.counter);

// Replay message1
const replayAttempt = bob.decrypt(message1, bobSession);
// Verify: Throws ReplayError "Counter already seen"
```

### Test tamper detection:
```typescript
const encrypted = alice.encrypt(Buffer.from('test'), aliceSession);
...
// Verify: Throws AuthenticationError "MAC verification failed"
const decrypted = bob.decrypt(encrypted, bobSession);
```

### Test key rotation:
```typescript
// Send 1001 messages (rotation threshold = 1000)
for (let i = 0; i < 1001; i++) {
  alice.encrypt(Buffer.from(`msg${i}`), aliceSession);
}

// Verify: Session key rotated after message 1000
// Verify: Old session key wiped from memory
```
...
- **Crypto primitives**: `packages/mathison-mesh/src/crypto/`
- **Handshake implementation**: X25519 ECDH + Ed25519 signatures
- **Encryption algorithm**: Node.js `crypto.createCipheriv('aes-256-gcm', ...)`
- **Key derivation**: `crypto.hkdf('sha256', sharedSecret, salt, info, 64)` → encryptKey + macKey
- **Replay protection**: In-memory sliding window + seen-set (size: 1000)
- **Key storage**: Identity keys persisted via `StorageAdapter`; session keys in-memory only
- **Secure wiping**: Use `crypto.timingSafeEqual` comparisons; zero buffers after use
