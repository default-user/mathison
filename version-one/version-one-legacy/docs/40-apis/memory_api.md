...
- **Application developers** building knowledge graphs, relationship networks, or structured data stores
- **Data engineers** integrating Mathison as a governed persistence layer
- **API integrators** who need CRUD operations on nodes, edges, and hyperedges
- **Auditors** requiring cryptographic receipts for all write operations

If you're executing long-running tasks, use the Jobs API instead. If you're interpreting text, use the OI API.

---

## Why This Exists

**Problem:** Traditional databases lack built-in governance, audit trails, and hypergraph support.

**Solution:** The Memory API provides:
- **Hypergraph model**: Nodes (entities) + Edges (binary relationships) + Hyperedges (N-ary relationships)
- **Governed writes**: All mutations protected by CIF (content validation) + CDI (capability checking)
- **Cryptographic receipts**: Every write produces a tamper-evident receipt with genome metadata
- **Idempotency**: Safe retries via `idempotency_key` (no duplicate writes)
- **Full-text search**: Query nodes by content

**Design choice:** Write operations are explicit and require consent; reads are permissive (but still governed).

---

## Guarantees / Invariants

1. **Write atomicity**: Node/edge creation either fully succeeds or fully fails (no partial writes)
2. **Idempotency**: Same `idempotency_key` → same result (cached response, no duplicate side effects)
3. **Receipt completeness**: Every write produces a receipt with `genome_id`, `genome_version`, `content_hash`, `timestamp`
4. **Referential integrity**: Edge creation fails if source or target node doesn't exist
5. **Fail-closed governance**: All operations denied if genome missing or CDI denies action
6. **JSON-only responses**: Enforced by governance pipeline (no HTML/plaintext leaks)

---

## Non-Goals

- **SQL/GraphQL query language**: Simple REST endpoints only (no complex query DSL)
- **Automatic schema enforcement**: Nodes store arbitrary JSON; app-level validation required
- **Distributed transactions**: Single-server only (no 2PC/Raft)
- **Real-time subscriptions**: Use polling or future gRPC streaming (HTTP API is request/response only)
- **Soft deletes**: No delete operations (append-only by design; mark nodes as "archived" in data)

---

## How to Verify

### Verify node creation and idempotency
```bash
# Create node
curl -X POST http://localhost:3000/memory/nodes \
  -H "Content-Type: application/json" \
  -d '{"id": "test-node", "type": "test", "data": {}, "idempotency_key": "key-1"}'
# Expected: 201 with receipt

# Retry with same idempotency_key
curl -X POST http://localhost:3000/memory/nodes \
  -H "Content-Type: application/json" \
  -d '{"id": "test-node", "type": "test", "data": {}, "idempotency_key": "key-1"}'
# Expected: 201 with same receipt (cached response)
```
...
curl -X POST http://localhost:3000/memory/edges \
  -H "Content-Type: application/json" \
  -d '{"from": "nonexistent", "to": "also-nonexistent", "type": "test", "idempotency_key": "key-2"}'
# Expected: 404 "Source node not found"
```
...
curl -X POST http://localhost:3000/memory/nodes \
  -H "Content-Type: application/json" \
  -d '{"type": "test", "data": {}, "idempotency_key": "key-3"}' | jq .receipt
# Expected: { genome_id: "sha256:...", genome_version: "1.0.0", ... }
```

### Verify search works
```bash
curl "http://localhost:3000/memory/search?q=test&limit=5"
# Expected: 200 with matching nodes
```
...
curl -X POST http://localhost:3000/memory/nodes \
  -H "Content-Type: application/json" \
  -d '{"type": "test"}'
# Expected: 400 "Missing or empty required field: idempotency_key"
```
...
**Location**: `/packages/mathison-server/src/routes/memory.ts`

**Key components**:
- **MemoryGraph**: In-memory hypergraph store (nodes, edges, hyperedges)
- **SearchIndex**: Full-text search over node data (simple term matching)
- **ActionGate**: Produces receipts for all write operations
- **CIF/CDI middleware**: Validates requests/responses before hitting MemoryGraph

**Storage**:
- In-memory by default (ephemeral)
- File-backed persistence: `/tmp/mathison/memory.json` (if enabled)
- Receipts stored separately via ActionGate

**Integration**:
- All routes wrapped by CIF ingress/egress middleware
- All routes protected by CDI action checks
- Write operations call ActionGate for receipt generation
